...
static void BCD_to_HEX(DS3231Proxy *ds3231hdl, uint8_t *data_array,
                       uint8_t array_length); /*turns the bcd time values from ds3231 into hex*/
static void HEX_to_BCD(DS3231Proxy *ds3231hdl, uint8_t *data_array,
                       uint8_t array_length); /*turns the hex numbers into bcd, to be written back into ds3231*/
static void ds3231_data_clone(DS3231Proxy *ds3231hdl, uint8_t option,
                              uint8_t *input_array); /*clones an array into one of 3 ..._registers_clone[], based on chosen option*/
                 ...
                 uint8_t run_command, uint8_t reset_state)
{
    ...
    if (((ds3231_init_status_report(ds3231hdl) == DS3231_NOT_INITIALIZED)
            && (reset_state == NO_FORCE_RESET)) || (reset_state == FORCE_RESET))
    {
...
uint8_t ds3231_run_command(DS3231Proxy *ds3231hdl, uint8_t command)
{
    ...
    {
    case CLOCK_RUN:
        DS3231_I2C_read_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                               DS3231_REGISTER_CONTROL,
                               &register_current_value);
        register_new_value = register_current_value & (~(1 << DS3231_BIT_EOSC));
        DS3231_I2C_write_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                                DS3231_REGISTER_CONTROL, &register_new_value);
...
{
    DS3231_I2C_read_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                           DS3231_REGISTER_CONTROL, &register_current_value);
...
void ds3231_reset(DS3231Proxy *ds3231hdl, uint8_t option)
{
    ...
    case SECOND:
        DS3231_I2C_write_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                                ...
                                &time_registers_clone[0]);
        break;
    case MINUTE:
        DS3231_I2C_write_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                                DS3231_REGISTER_MINUTES,
                                &time_registers_clone[1]);
        break;
    case HOUR:
        time_registers_clone[2] &= (~(1 << DS3231_BIT_12_24)); /*to turn on 24 hours format by default*/
        DS3231_I2C_write_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                                DS3231_REGISTER_HOURS,
                                &time_registers_clone[2]);
        break;
    case DAY_OF_WEEK:
        DS3231_I2C_write_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                                DS3231_REGISTER_DAY_OF_WEEK,
                                &time_registers_clone[3]);
        break;
    case DATE:
        DS3231_I2C_write_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                                DS3231_REGISTER_DATE, &time_registers_clone[4]);
        break;
    case MONTH:
        time_registers_clone[5] &= (~(1 << DS3231_BIT_CENTURY));
        DS3231_I2C_write_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                                DS3231_REGISTER_MONTH,
                                &time_registers_clone[5]);
        break;
    case YEAR:
        DS3231_I2C_write_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                                DS3231_REGISTER_YEAR, &time_registers_clone[6]);
        break;
    case CONTROL:
        DS3231_I2C_read_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                               DS3231_REGISTER_CONTROL,
                               &register_current_value); /*in order to preserve running state (RUN or HALT)*/
        register_new_value = (register_current_value & (1 << DS3231_BIT_EOSC))
                | (register_default_value[0X0E] & (~(1 << DS3231_BIT_EOSC)));
        DS3231_I2C_write_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                                DS3231_REGISTER_CONTROL, &register_new_value);
        break;
    case CONTROL_STATUS:
        DS3231_I2C_read_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                               DS3231_REGISTER_CONTROL_STATUS,
                               &register_current_value); /*in order to preserve OSF flag*/
        register_new_value = (register_current_value & (1 << DS3231_BIT_OSF))
                | (register_default_value[0X0F] & (~(1 << DS3231_BIT_EOSC)));
        DS3231_I2C_write_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                                DS3231_REGISTER_CONTROL_STATUS,
                                &register_new_value);
        break;
    case ALARM1:
        DS3231_I2C_write_multi(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                               DS3231_REGISTER_ALARM1_SECONDS,
                               &alarm1_registers_clone[0], 4);
        break;
    case ALARM2:
        DS3231_I2C_write_multi(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                               DS3231_REGISTER_ALARM2_MINUTES,
                               &alarm2_registers_clone[0], 3);
        break;
    case ALARMS:
        DS3231_I2C_write_multi(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                               DS3231_REGISTER_ALARM1_SECONDS,
    ...
    case ALL:
        ...
        DS3231_I2C_write_multi(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                               DS3231_REGISTER_SECONDS,
                               &time_registers_clone[0], 7); /*to reset all the TIME registers*/
        ...
        DS3231_I2C_read_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                               DS3231_REGISTER_CONTROL,
                               &register_current_value); /*to preserve run_status, either RUN or HALT*/
        register_new_value = (register_current_value & (1 << DS3231_BIT_EOSC))
                | (register_default_value[0X0E] & (~(1 << DS3231_BIT_EOSC)));
        DS3231_I2C_write_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                                DS3231_REGISTER_CONTROL, &register_new_value);
...
uint8_t ds3231_read(DS3231Proxy *ds3231hdl, uint8_t option, uint8_t *data_array)
{
    ...
    case HOUR:
        DS3231_I2C_read_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                               DS3231_REGISTER_HOURS, &register_current_value);
        *data_array = register_current_value;
        BCD_to_HEX(ds3231hdl, data_array, 1);
        break;
    case DAY_OF_WEEK:
    ...
    case DATE:
        DS3231_I2C_read_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                               DS3231_REGISTER_DATE, &register_current_value);
        *data_array = register_current_value;
        BCD_to_HEX(ds3231hdl, data_array, 1);
        break;
    case MONTH:
        DS3231_I2C_read_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                               DS3231_REGISTER_MONTH, &register_current_value);
        *data_array = register_current_value;
        BCD_to_HEX(ds3231hdl, data_array, 1);
        break;
    case YEAR:
        DS3231_I2C_read_single(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                               DS3231_REGISTER_YEAR, &register_current_value);
        *data_array = register_current_value;
        BCD_to_HEX(ds3231hdl, data_array, 1);
        break;
    case CONTROL:
    ...
    case AGING_OFFSET:
        ...
        *data_array = register_current_value;
    case TIME:
        DS3231_I2C_read_multi(ds3231hdl->ds3231I2C, DS3231_I2C_ADDRESS,
                              DS3231_REGISTER_SECONDS, data_array, 7);
                       ...
                       uint8_t array_length)
{
    ...
    {
        data_array[index] = ((data_array[index] >> 4) << 1)
                + ((data_array[index] >> 4) << 3) + (data_array[index] & 0X0F);
    }
}
...
#pragma CODE_SECTION(ds3231Thread, ".TI.ramfunc")
void* ds3231Thread(void *arg0)
{
    sem_wait(&initDs3231Done);
    DS3231Event *event;

    while (1)
    {
        xQueueReceive(ds3231EventQueue, event, portMAX_DELAY);
        switch (event->eventType)
        {
        case READ_TIME:
            ds3231_read(ds3231hdl, TIME, time_array);
            ds3231hdl->sec = time_array[0];
            ds3231hdl->min = time_array[1];
            ds3231hdl->hour = time_array[2];
            ds3231hdl->dow = time_array[3];
            ds3231hdl->day = time_array[4];
            ds3231hdl->month = time_array[5];
            ds3231hdl->year = time_array[6];
            break;
        default:
            break;
        }
